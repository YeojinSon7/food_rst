...
def run_app_home():
    ...
    df_ex= pd.DataFrame({
    'columns':['FCLTY_NM','RDNMADR_NM','CTPRVN_CD','SIGNGU_CD','FCLTY_LO','FCLTY_LA','AREA_NM','REPRSNT_MENU_NM','MENU_PRC','BASE_DE'],
    'explain':['시설명','도로명주소','시도코드','시군구코드','시설경도','시설위도','지역명','대표메뉴명','메뉴가격','기준일자'],
})  
    st.divider()
    st.markdown('-  원본 데이터의 :orange[CTPRVN_CD, SIGNGU_CD, FCLTY_LO, FCLTY_LA, BASE_DE] 컬럼은 이용자에게 필요없다고 판단하여 제외시켰고, :blue[FCLTY_NM]::yellow[식당이름],RDNMADR_NM:도로명주소,AREA_NM:지역,REPRSNT_MENU_NM:대표메뉴,MENU_PRC:가격으로 바꿔서 이용자가 보기 편하게 만들었습니다 ')
